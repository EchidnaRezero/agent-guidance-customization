...
Make superpowers skills work in the Codex App's sandboxed worktree environment without breaking existing Codex behavior in other runtimes.
...
- **IN_LINKED_WORKTREE:** `GIT_DIR != GIT_COMMON` — the agent is in a worktree created by something else (Codex App, previous skill run, or the user)
...
|---|---|---|---|
| No | No | Codex CLI / normal git | Full skill behavior (unchanged) |
...
- The 4-option finishing menu — preserved exactly for Codex CLI
...
### Automated (run in a full-access Codex environment after implementation)
...
- Existing Codex skill-triggering tests still pass
- Existing subagent-driven-development integration tests still pass
- Normal Codex CLI session: full worktree creation + 4-option finishing still works
