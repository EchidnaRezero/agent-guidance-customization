...
**If `GIT_DIR` differs from `GIT_COMMON`:** You are already inside a linked worktree (created by the Codex App, a previous skill run, or the user). Do NOT create another worktree. Instead:
...
Note: these tests require a full-access Codex environment. If not available, document that regression tests should be run manually.
