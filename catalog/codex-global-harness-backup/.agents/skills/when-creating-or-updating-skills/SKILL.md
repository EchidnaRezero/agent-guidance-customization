...
For local Codex harness exceptions to this upstream guidance, read `references/local-skill-creator-exceptions.md`.
...
### Korean Companion Requirement

When creating or updating a skill, treat the Korean companion file as required alongside `SKILL.md`:
...
When creating or updating a skill that should also be readable by Korean users, create or update a matching `SKILL_KR.md` companion in the same folder. Keep the Korean companion synchronized with the canonical `SKILL.md` whenever the skill's structure, meaning, or instructions change.
...
- "Where should I create this skill? If you do not have a preference, I will place it in `skills/` at the current Codex root so Codex can discover it automatically."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `skills/` at the current Codex root so the skill is auto-discovered.
...
After creating the skill, also create the Korean companion file with `_KR` in the filename before considering initialization complete.
...
For local Codex harness exceptions to this rule, read `references/local-skill-creator-exceptions.md`.
